...
export default function DocumentEntry({ kegiatanId, scope }: DocumentEntryProps) {
  ...
  const fetchInitialData = async () => {
    ...
    } else if (profile.role !== 'admin' && profile.role !== 'direktur') {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Arsip Dokumen</h2>
          {categories.length > 0 ? (
            ...
            <p className="text-slate-400 text-xs mt-2 italic font-medium">Belum ada kategori tersedia.</p>
          )}
        </div>
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className="flex items-center justify-center space-x-2 rounded-2xl border-2 border-slate-100 bg-white px-6 py-4 text-xs font-black text-slate-600 hover:bg-slate-50 transition-all active:scale-95 uppercase tracking-widest shadow-sm disabled:opacity-50"
          >
            <FileDown size={18} />
            <span>Unduh Laporan</span>
          </button>
          {userProfile?.role !== 'direktur' && categories.length > 0 && (
            <button 
